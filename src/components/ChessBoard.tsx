...
export const ChessBoard: React.FC<ChessBoardProps> = ({ onGameEnd }) => {
    ...
    return (
      ...
      >
        {piece && (
          ...
          >
            {pieceSymbols[piece.color === 'w' ? piece.type.toUpperCase() : piece.type.toUpperCase()] || ''}
          </motion.span>
        )}
        {isPossibleMove && (
          <motion.div 
  ...
  return (
    <div className="relative space-y-6">
      ...
      {/* Board Coordinates */}
      <div className="absolute left-0 top-0 h-full flex flex-col text-amber-700 font-bold text-sm pointer-events-none z-10">
        {[8, 7, 6, 5, 4, 3, 2, 1].map(num => (
          <div key={num} className="flex-1 flex items-center pl-1">{num}</div>
        ))}
      </div>
